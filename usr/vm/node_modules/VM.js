...
function startVM(vmobj, extra, log, callback)
{
	...
	// disable TSC for KVM live migration
	additional_cpu_flags = ',-tsc';
    if (vmobj.hasOwnProperty('cpu_type')) {
		if (vmobj.cpu_type == 'qemu64') {
			cmdargs.push('-cpu', vmobj.cpu_type + additional_cpu_flags);
		} else {
			cmdargs.push('-cpu', vmobj.cpu_type);
		}
    } else {
        cmdargs.push('-cpu', 'qemu64' + additional_cpu_flags);
    }
